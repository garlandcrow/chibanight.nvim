...
function M.setup()
  ...
  for extra, info in pairs(M.extras) do
    package.loaded["chibanight.extra." .. extra] = nil
    local plugin = require("chibanight.extra." .. extra)
    for style, style_name in pairs(styles) do
      config.setup({ style = style })
      local colors = require("chibanight.colors").setup({ transform = true })
      local fname = extra .. "/chibanight_" .. style .. "." .. info.ext
      colors["_upstream_url"] = "https://github.com/folke/chibanight.nvim/raw/main/extras/" .. fname
